...
using UnityEditor;
using UnityEngine;
using Menu = Innoactive.Hub.Training.Editors.Configuration.Menu;
    ...
    public class AllowedMenuItemsWindow : EditorWindow
    {
        ...
        private static void InitializeLists()
        {
            ...
            foreach (KeyValuePair<string, bool> behavior in EditorConfigurator.Instance.AllowedMenuItemsSettings.SerializedBehaviorSelections)
            {
                ...
                string displayedName = (isTypeValid) ? ((Menu.Item<IBehavior>)ReflectionUtils.CreateInstanceOfType(type)).DisplayedName.text : behavior.Key;
            ...
            foreach (KeyValuePair<string, bool> condition in EditorConfigurator.Instance.AllowedMenuItemsSettings.SerializedConditionSelections)
            {
                ...
                bool isTypeValid = type != null;
                string displayedName = (isTypeValid) ? ((Menu.Item<ICondition>)ReflectionUtils.CreateInstanceOfType(type)).DisplayedName.text : condition.Key;
